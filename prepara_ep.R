...
individual_03.hoy <- 
  ...
  mutate(AGLOMERADO = case_when(
    ...
    AGLOMERADO == 31 ~ 'USHUAIA - RIO GRANDE',
    AGLOMERADO == 32 ~ 'CABA',
...
canastas <- read.csv('canastas.csv')
canastas <- canastas %>% 
  mutate(
    region = case_when(
      region == 'Noreste' ~ 'NEA',
...
individual_03.hoy <- calcula_pobreza_hogar(individual_03.hoy,basket = canastas,print_summary = FALSE)
